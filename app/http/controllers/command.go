...
func GetCommands(request contracts.HttpRequest, guard contracts.Guard) any {
	...
	list, total := models.Commands().
		...
		OrderByDesc("id").
		When(request.GetString("name") != "", func(q contracts.Query[models.Command]) contracts.Query[models.Command] {
			return q.Where("name", "like", "%"+request.GetString("name")+"%")
		}).
		When(request.GetString("user") != "", func(q contracts.Query[models.Command]) contracts.Query[models.Command] {
			return q.Where("user", "like", "%"+request.GetString("user")+"%")
		}).
		When(request.GetString("step") != "", func(q contracts.Query[models.Command]) contracts.Query[models.Command] {
